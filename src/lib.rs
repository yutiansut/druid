...
pub trait WidgetInner<T> {
    ...
    fn event(&mut self, event: &Event, ctx: &mut EventCtx, data: &mut T, env: &Env) -> Option<Action>;
}
...
impl<T: PartialEq + Clone> UiState<T> {
    pub fn new(root: impl Into<BoxedWidget<T>>, data: T) -> UiState<T> {
        UiState {
            root: root.into(),
            data,
...
impl<T: PartialEq + Clone + 'static> WinHandler for UiMain<T> {
    ...
    fn paint(&self, piet: &mut Piet) -> bool {
        ...
        let size = state.root.layout(&mut layout_ctx, &bc, &state.data, &env);
        ...
        state.root.paint(&mut paint_ctx, &state.data, &env);
        false
    }
